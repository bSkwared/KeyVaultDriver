...
int  init_vault (struct key_vault *v, int size) {
   ...
   v->num_users = 0;
   v->ukey_data = calloc(size, sizeof(struct kv_list_h));
...
int  insert_pair (struct key_vault *v, int uid, char *key, char *val) {
   ...
   if (user->num_keys == 0) {
      user->data = calloc(MAX_KEY_USER, sizeof(struct kv_list));
...
int  retrieve_val (struct key_vault *v, int uid, char *key, 
	...
	struct kv_list *l = find_key(v, uid, key, &is_last_key);
 ...
 *            is_last_key to TRUE if the returned key is the last one of the
 *            user's set. uid is one-indexed.
 */
struct kv_list* find_key (struct key_vault *v, int uid, char *key, 
   ...
   if (i == user->num_keys) return NULL;
	...
	/* otherwise, set is_last_key and return l as the pointer to the kv_list */
	if (i == user->num_keys-1) *is_last_key = TRUE;
	return &l[i];
}
...
struct kv_list* next_key (struct key_vault *v, int uid, struct kv_list *l) {
	...
	int is_last_key;
	l = find_key(v, uid, l->kv.key, &is_last_key);

	/* if this key is the last in the array of keys for this user, return NULL */
	if (is_last_key) return NULL;

	/* othereise, return the next key in the array */
	return ++l;
}
...
struct kv_list* prev_key (struct key_vault *v, int uid, struct kv_list *l) {
	...
	if (uid < 1 || uid > v->num_users || l==v->ukey_data[uid-1].data) return NULL;

	/* otherwise, find first (perhaps ony) key of list (is_last_key unused) */
	int is_last_key;
	l = find_key(v, uid, l->kv.key, &is_last_key);

	/* otherwise, return the last key in the prev list */
	l = get_last_in_list(--l);
...
int  insert_in_list (struct kv_list *l, char *key, char *val) {
   ...
   if (l->kv.key[0] != '\0') {
		...
		struct kv_list *last = l;
		while (l->next != NULL) l = l->next;

      /* add the new key-value pair */
      l->next = malloc(sizeof(struct kv_list));
